...
def plotspec(specData, bandNames, limits, objID, plotInstructions, specnames,figNum=1):
    ...
    for bandIdx, band in enumerate(bandNames):
        ...
        # 5) Label Objects =======================================================
        if bandIdx == 2:
            xpos = 0.93  # in wavelength units
            ypos = 2.4  # in flux units
            subPlot.text(xpos, ypos + 1.1, '0408-1450', color=colors[0], size = 10.0)
            subPlot.text(xpos, ypos, '2137+0808', color=colors[1], size = 10)
            subPlot.text(xpos, ypos - 1.1, '0825+2115', color=colors[2], size = 10)
...
for idxTp, spTp in enumerate(SPTYPES):
    ...
    for bdIdx, band in enumerate(BANDS):
        ...
        templRaw = ad.read('../templates/' + fileNm, delimiter='\t', \
                           format='no_header')
...
plt.savefig('../plots/templates-stdsL2L7.pdf', dpi=300)